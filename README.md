...
2. Prepare prompts using Qwen2.5-Omni-7B and llama.cpp . Start the server: (by default it listens host 127.0.0.1, port 8080)
    ...
    Alternatively, you can use https://github.com/Skelp/Qwen2.5-Omni-GPTQ-4bit-audio-metadata/blob/feature/low-vram-audio-metadata/generate_audio_metadata.py

    Each prompt is a list of tags separated by comma space `, `. Currently the base model does not support natural language.

3. Prepare lyrics:

    TODO: First try extracting embedded lyrics. Then use Whisper, or query online databases.
